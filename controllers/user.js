...
exports.actions = async(req, res) => {
  ...
  else
  {
    ...
    else
    {
      ...
      if (sockets[userIdT])
        sockets[userIdT].emit('notification',{not: `${req.session.userName} liked you`});
    }
    if(action == 'love')
    {
...
exports.getPublicProfile = async(req, res) => {
  ...
  // get the blocked users  from me
  var blockedUsers = (await user.blockedUsersNameF(req.session.userId))[0];
  var found = _.find(blockedUsers, function(o) { return o.userName == userToFind; });
  // get the blocked users To
  var blockedUsers2 = (await user.blockedUsersNameT(req.session.userId))[0];

  var found2 = _.find(blockedUsers2, function(o) { return o.userName == userToFind; });
  // check it's valid 
  ...
  if (foundedUser.length == 0)
    return res.redirect('/user/home');
  else if(found != undefined || found2 != undefined)
    return res.redirect('/user/home');
  else
  {
...
exports.postProfileData = async(req, res) => {
  ...
  {
    return res.json([{msg: "Please send all fields."}]);
  }
